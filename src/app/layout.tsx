...
export const metadata: Metadata = {
  title: "CarShop - Quality Cars and Services",
  description: "Explore CarShop for premium cars and exceptional services tailored to your needs.",
  openGraph: {
    title: "CarShop - Quality Cars and Services",
    description: "Explore CarShop for premium cars and exceptional services tailored to your needs.",
    url: "https://carshop.com",
    siteName: "CarShop",
    images: [{
      url: "https://images.pexels.com/photos/120049/pexels-photo-120049.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
      alt: "CarShop showroom",
    }],
    type: "website",
  },
  twitter: {
...
(function() {
  ...
  // Helper function to extract original URL from Next.js optimized images
  const extractOriginalUrl = (url) => {
    if (!url) return url;

    // Check if it's a Next.js optimized image (_next/static or _next/image)
    if (url.includes('/_next/')) {
      try {
        const urlObj = new URL(url);
        // Extract the 'url' parameter which contains the original path
  ...
  const getElementInfo = (element) => {
    ...
    if (tagName === 'img') {
      // Extract original URL from Next.js optimized images
    ...
    if (backgroundImage && backgroundImage !== 'none') {
      const urlMatch = backgroundImage.match(/url\(['"]?([^'")]+)['"]?\)/);
      if (urlMatch) {
        // Extract original URL from Next.js optimized background images
  ...
  const saveChangeToStorage = (change) => {
    try {
      ...
      // Deduplicate by oldValue + sectionId combination
      // This ensures that if the same text is changed multiple times, only the latest change is kept
      const filteredChanges = existingChanges.filter(c => {
        // Keep changes that don't match this oldValue + sectionId combination
